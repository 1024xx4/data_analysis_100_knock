...
考えられるあらゆる組み合わせの中から、**成約条件**を満たしたうえで**目的関数**を最小化（最大化）する組み合わせを選択する、というのが、
最適化問題の大きな流れ。

目的関数と制約条件さえ明確にできれば、最適化計算 Tool によって手計算よりも遥かに楽に解を求めることが可能。
最適化問題の中でも**線形最適化**に定型化できるものは比較的短時間に解を求めれることができるとわかっている。  
**※ 線形最適化  
ex). 輸送最適化問題など**
